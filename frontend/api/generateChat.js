...
export default async function generateChat(req, res) {
  ...
  try {
    ...
    const chatCompletion = await openai.chat.completions.create({
      model: "gpt-3.5-turbo",
      messages: [
        { role: "system", content: "You are a Mexican Spanish short story generator. You will always provide a story title first, followed by a '|' and then the story. The story should never be more than 250 words, and it must incldue all of these words: " + words + "."},
        {
          role: "user",
          content: prompt,
        },
  ...
  switch (skillLevel) {
    case "beginner":
      prompt =
        ...
        "Create a story using elementary spanish, where most verbs are in the present tense."
      break;
    case "intermediate":
      prompt =
        ...
        "Create a story using intermediate spanish, where verbs can be of different tenses."
      break;
    case "advanced":
      prompt =
        ...
        "Create a story using advanced spanish, multiple different verb tenses and large vocabulary."
      break;
    default:
